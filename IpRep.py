...
def content_test(url, Dirtyip):
    """
    Test the content of url's response to see if it contains Dirtyip.
        Args:
            url -- the URL to request data from
            Dirtyip -- the IP address in question
        Returns:
            Boolean
    """
    try:
        ...
        matches = retcode == 200
        matches = matches and re.findall(Dirtyip, html_content)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Is This IP Dirty?')
    parser.add_argument('-i', '--ip', help='IP address to check')
    parser.add_argument('--success', help='Also display Gd', required=False, action="store_true")
    args = parser.parse_args()

    if args is not None and args.ip is not None and len(args.ip) > 0:
        Dirtyip = args.ip
    else:
        my_ip = urlopen('http://icanhazip.com').read().strip()
        print(type(str(my_ip)))

        print(blue('Check IPS against popular ip and DNS blacklists'))
        print(blue('A quick and dirty script by Oliver Cecil Joseph(OCJ) \n'))
        ...
        if resp.lower() in ["yes", "y"]:
            ...
            if Dirtyip is None or Dirtyip == "":
                sys.exit("No IP address to check.")
    #IP INFO
    reversed_dns = socket.getfqdn(Dirtyip)
    print(Dirtyip)
    geoip = urlopen('http://api.hackertarget.com/geoip/?q='+Dirtyip).read().strip()
    print(blue('\nThe FQDN for {0} is {1}\n'.format(Dirtyip, reversed_dns)))
    ...
    for url, succ, fail, mal in URLS:
        if content_test(url, Dirtyip):
        	if args.success:
        		print(green('{0} {1}'.format(Dirtyip, succ)))
        		Gd =Gd + 1
        else:
            print(red('{0} {1}'.format(Dirtyip, fail)))
            Dirty = Dirty + 1
    Dirty = Dirty
    Gd = Gd
    for bl in bls:
        try:
            ...
            query = '.'.join(reversed(str(Dirtyip).split("."))) + "." + bl
            my_resolver.timeout = 5
            my_resolver.lifetime = 5
            answers = my_resolver.query(query, "A")
            answer_txt = my_resolver.query(query, "TXT")
            print (red(Dirtyip + 'is listed in ' + bl)
                   + ' (%s: %s)' % (answers[0], answer_txt[0]))
            Dirty = Dirty + 1
        except dns.resolver.NXDOMAIN:
            print (green(Dirtyip + 'is not listed in'+bl))
            Gd = Gd + 1
    ...
    print(red("\n{0} is on {1}/{2} {3}'%'blacklists.\n".format(Dirtyip, Dirty, (Gd+Dirty),((Dirty*100)/(Gd+Dirty)))))
